...
filename = sprintf('%s/%d_goodlist_check_300m_%d_%d.txt',outpath,varargin{3},varargin{1},varargin{2});
...
for i=start_id:end_id
    search_path = [path,'/',num2str(i,'%03d'),'*']
	string_regionname=dir([path,'/',num2str(i,'%03d'),'*'])
    if sensor == 1
        out_folder = [result,'/',string_regionname.name,'/','SETSM_SDM_300']
        %out_folder = [result,'/','188_Steenby','SETSM_SDM_300']
    else
	    out_folder = [result,'/',string_regionname.name,'/','SETSM_SDM_300_new']
	end
